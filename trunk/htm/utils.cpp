...
double rgamma(double a, double b){
    return gsl_ran_gamma_mt(RANDOM_NUMBER_GENERATOR, a, b);
}

double rbeta(double a, double b){
    return gsl_ran_beta(RANDOM_NUMBER_GENERATOR, a, b);
}

unsigned int rbernoulli(double p){
    return gsl_ran_bernoulli(RANDOM_NUMBER_GENERATOR, p);
}
...
void rshuffle(void* base, size_t n, size_t size){
    gsl_ran_shuffle(RANDOM_NUMBER_GENERATOR, base, n, size);
}

unsigned long int runiform_int(unsigned long int n){
    return gsl_rng_uniform_int(RANDOM_NUMBER_GENERATOR, n);
}

void init_random_generator(unsigned long int random_seed){
	RANDOM_NUMBER_GENERATOR = gsl_rng_alloc (gsl_rng_mt19937);     		// pick random number generator
	gsl_rng_set (RANDOM_NUMBER_GENERATOR, random_seed);                  // sets seed
}
